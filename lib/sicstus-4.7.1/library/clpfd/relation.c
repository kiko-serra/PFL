...
static int bool_channel_eq(Wam wam, struct bool_channel_data *pdata,int sense)
{
  ...
  if (!ent) {			/* update "prev domain" - slight overkill */
    /* GC invariant: ptr from older heap segment to newer MUST be trailed */
    ...
    slot = TagREF(TagToArg(X(0),6));
    CTagToArg(X(0),6) = slot;
    BindHVA(slot, newdom);
  }
 ret:
  pdata->top = pdata->ntargets;
...
SP_BOOL propagate_disequations(Wam wam, TAGGED list)
{
  ...
  while (TagIsLST(X(0))) {
    ...
    fd_update_mutable(wam,count-IStep(1),countm);
