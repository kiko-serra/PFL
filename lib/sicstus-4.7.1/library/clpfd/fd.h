...
#define Pdata(What,Handle) ((What *)fd_perm_data(wam, Handle))
...
extern void *fd_perm_data(Wam wam,TAGGED);
...
#define FdMemRequireHeap(N,Arity)		\
SetGCHazard;					\
RequireHeap(N,Arity);				\

#define FdMemRequireHeap1(N,A,Arity)		\
SetGCHazard;					\
RequireHeap1(N,A,Arity);			\

#define FdMemRequireHeap2(N,A,B,Arity)		\
SetGCHazard;					\
RequireHeap2(N,A,B,Arity);			\

#if SP_ASSERTIONS

#define ClearGCHazard				\
fd.gc_hazard = FALSE;				\

#define SetGCHazard				\
fd.gc_hazard = TRUE;				\

#define CheckGCHazard				\
SP_ASSERT(!fd.gc_hazard);			\
...
#define Deref1(To,From) {(To) = (From); if (IsVar(To)) (To) = CTagToPointer(To);}
...
/*	 v(1,0,'$mutable'(dom([[inf|sup]],inf,sup,sup),0),
	       ...
	       '$mutable'([v(...)]))
*/
#define FD_ATTR_VAR_OFFSET 2
#define FD_ATTR_MIN_OFFSET 20
#define FD_ATTR_MAX_OFFSET 21
#define FD_ATTR_SIZE_OFFSET 22
#define FD_ATTRIBUTE_SIZE 50
#define INT_ATTRIBUTE_SIZE 17

/* support unification of dvars */
#define RefMutable(Mut) CTagToArg((Mut),1)

#define AttrToVar(Attr,To)				\
{							\
  (To)=CTagToArg(Attr,2);				\
}
...
#define AttrToAliasM(Attr,To)				\
{							\
  (To)=CTagToArg(Attr,5);				\
}

#define AttrAFC(Attr) CTagToArg(Attr,6)
...
{									\
  ...
  if (m_e1==Sup)							\
    m_e1 = SupAsINT+1;							\
  else if (Tge(m_e1,SupAsINT-IStep(1))) /* SPRM 13738 */		\
    m_e1 = SupAsINT;							\
  else									\
  ...
  if (m_e2==Sup)							\
    m_e2 = SupAsINT+1;							\
  else if (Tge(m_e2,SupAsINT-IStep(1)))	/* SPRM 13738 */		\
    m_e2 = SupAsINT;							\
  else									\
...
#define FD_LIST_VAL_IX        19
#define FD_LIST_VAL_DAEMON    20
#define FD_LIST_VAL_GLOBAL    21
#define FD_LISTS_SIZE         22

#define FD_CHECK_ARGUMENT_NEED (FD_ATTRIBUTE_SIZE + ARITYLIMIT + 4)
#define FD_LINK_NEED (FD_LISTS_SIZE+2)

#define FD_QUEUE_OFFSET  0
#define FD_QUEUE_IMP     1
#define FD_QUEUE_DISEQ   2
#define FD_QUEUE_WATCHER 3
...
extern SP_integer fd_middle(TAGGED set);
extern SP_integer fd_median(TAGGED set);
...
extern TAGGED fd_check_argument(Wam wam, TAGGED argument, TAGGED min, TAGGED max, TAGGED size, SP_BOOL is_fd);
extern int fd_compare(TAGGED d1, TAGGED d2);
extern int fd_compare_interval(TAGGED d1, TAGGED min, TAGGED max);
extern int fd_compare_intervals(TAGGED b1, TAGGED e1, TAGGED b2, TAGGED e2);
extern SP_BOOL fd_intersect(TAGGED d1, TAGGED d2);
extern SP_BOOL fd_intersect_interval(TAGGED d1, TAGGED min, TAGGED max);
extern SP_BOOL fd_intersect_intervals(TAGGED b1, TAGGED e1, TAGGED b2, TAGGED e2);
...
extern void fd_tell_value(Wam wam, TAGGED value);
extern void fd_tell_interval(Wam wam, TAGGED min, TAGGED max, int why);
extern void fd_tell(Wam wam, TAGGED new);
extern void fd_tell_unsafe(Wam wam, TAGGED new);
extern TAGGED fd_link(Wam wam, TAGGED var, TAGGED key, TAGGED item, TAGGED trigger);
extern void fd_link_attr(Wam wam, TAGGED attr, TAGGED key, TAGGED item, TAGGED trigger);
extern void request_tell_value_fast(Wam wam, TAGGED value);
extern void request_tell_value(Wam wam, TAGGED value);
extern void request_tell_bool(Wam wam, TAGGED attr, TAGGED var, TAGGED tval);
...
{									\
  ...
  fd_update_mutable(wam,RefMutable(mutable)|IStep(1),mutable); /* STATUS: enqueued */ \
  cur->hint |= (1U << (ix));						\
}									\
...
#define fd_enqueue_offset(wam,dom,why,value,list)	\
{							\
...
extern SP_BOOL SPCDECL fd_labeling_hook (Wam wam);
...
{							\
  ...
  DomFromAttr(m_tmp,m_tmp); /* get dom/4 term */	\
  (MIN) = DomainMin(m_tmp);				\
  (MAX) = DomainMax(m_tmp);				\
}							\
...
extern void
fd_labeling(Wam wam, void *altptr);
...
extern struct instance *get_dbref(Wam wam, TAGGED ref);
