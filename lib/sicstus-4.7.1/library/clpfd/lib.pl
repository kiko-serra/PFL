...
't=u+c'(T,U,C) :-
	Constraint = 't=u+c'(T,U,C),
	check_arguments(Constraint, Attv),
	equate_and_propagate(Constraint, Attv).
...
/****************************************************************/
/* all_different/[1,2], all_distinct/[1,2]                      */
/****************************************************************/

all_different(Xs) :-
        OptionsArgNo = 0,
	all_different(Xs, [], opt(local,val,[],[],0,false,false), all_different(Xs), OptionsArgNo).

all_different(Xs, Opt) :-
        OptionsArgNo = 2,
	all_different(Xs, Opt, opt(local,val,[],[],0,false,false), all_different(Xs,Opt), OptionsArgNo).

all_distinct(Xs) :-
        OptionsArgNo = 0,
	all_different(Xs, [], opt(global,dom,[],[],0,false,false), all_distinct(Xs), OptionsArgNo).

all_distinct(Xs, Opt) :-
        OptionsArgNo = 2,
	all_different(Xs, Opt, opt(global,dom,[],[],0,false,false), all_distinct(Xs,Opt), OptionsArgNo).
...
all_different(Xs, Options, Opt0, Goal, OptionsArgNo) :-
	...
	length(Xs, N),
	Opt = opt(Cons,On,_,Eqs,Hack,Circuit,SubCircuit),
	dvar_list_susp(Xs, Vec, On, Goal, 1, Susp, Susp1),
	(   nonvar(Susp), Susp = [F|_] -> functor(F, SuspF, 1)
	;   SuspF = none
	),
	(   foreach(X,Xs),
	    foreach(none(X),SuspNone)
	do  true
	),
	(   foreach(X,Xs),
	    foreach(val(X),SuspVal)
	do  true
	),
	(   SubCircuit = true
	->  Flag=0,
	    ...
	    domain(Xs, Set),
	    (   N<2 -> true
	    ;   fd_global_internal(subcircuit(Xs), state(Vec,0,_,0), SuspNone,
	...
	->  Flag=1,
	    ...
	    (   N<2 -> true
	    ;   not_self(Xs, 0),
		fd_global_internal(circuit(Xs), state(Vec,0,_,0), SuspNone,
				   _, clpfd:Goal, 4) % non-idempotent
	    )
	;   Flag=0
	),
...
all_diff_option(on(On), opt(Cons,_,Cost,Eqs,Hack,Circ,SubCirc), opt(Cons,On,Cost,Eqs,Hack,Circ,SubCirc)) :-
	on(On, _, _, _).
all_diff_option(consistency(Arg), opt(_,_,Cost,Eqs,Hack,Circ,SubCirc), opt(Cons,On,Cost,Eqs,Hack,Circ,SubCirc)) :-
	consistency_option(Arg, Cons, On).
all_diff_option(cost(V,Mat), opt(Cons,On,_,Eqs,Hack,Circ,SubCirc), opt(Cons,On,cost(V,VM,Mat),Eqs,Hack,Circ,SubCirc)) :-
	arg_attribute(V, VM, 0, 3).
all_diff_option(L #= R, opt(_,_,Cost,Eqs,Hack,Circ,SubCirc), opt(Cons,On,Cost,[L #= R|Eqs],Hack,Circ,SubCirc)) :-
	consistency_option(bound, Cons, On).
all_diff_option(circuit(Circ), opt(Cons,On,Cost,Eqs,Hack,_,SubCirc), opt(Cons,On,Cost,Eqs,Hack,Circ,SubCirc)) :-
	bool_option(Circ, _).
all_diff_option(subcircuit(SubCirc), opt(Cons,On,Cost,Eqs,Hack,Circ,_), opt(Cons,On,Cost,Eqs,Hack,Circ,SubCirc)) :-
	bool_option(SubCirc, _).
% forbidden values optimization, undocumented so far
all_diff_option(hack(Hack), opt(Cons,On,Cost,Eqs,_,Circ,SubCirc), opt(Cons,On,Cost,Eqs,F,Circ,SubCirc)) :-
	bool_option(Hack, F).

/****************************************************************/
/* all_different_except_0/[1,2]                                 */
/* all_distinct_except_0/[1,2]                                  */
/****************************************************************/

all_different_except_0(Xs) :-
	all_different_except_0(Xs, opt(local,val,[],[],0,false,false), all_different_except_0(Xs), all_different).

all_distinct_except_0(Xs) :-
	all_different_except_0(Xs, opt(global,dom,[],[],0,false,false), all_distinct_except_0(Xs), all_distinct).

all_different_except_0(Xs, Opt, Goal, Rewritten) :-
	must_be_list_of_finite_dvar(Xs, Goal, 1),
	(   foreach(X1,Xs),
	    fromto(-1,UB1,UB2,U)
	do  fd_max(X1, Xmax),
	...
	Goal2 =.. [Rewritten,Ys],
	all_different(Ys, [], Opt, Goal2, 0).

swap_values(Xs, Ys, U) :-
	(   foreach(X,Xs),
	    foreach(Y,Ys),
	    count(V,U+1,_)
	do  swap_values(X, Y, 0, V)
	).

swap_values(X, Y, I, J) +:
	X in (dom(Y) /\ \({J})) \/ ((dom(Y) /\ {J}) ? {I}),
	Y in (dom(X) /\ \({I})) \/ ((dom(X) /\ {I}) ? {J}).
...
element(Index, Xs, Value) :-
	...
	;   ground(Xs) ->
	    ...
	    fd_global(Goal, f(Index-IndexM,Value-ValueM,Xs,3,_Handle,0), [dom(Index),dom(Value)])
	;   arg_attribute(Index, IndexM, Goal, 1),
	    arg_attribute(Value, ValueM, Goal, 3),
	    dvars_are_bool([Value|Xs], IsB),
	    Options is IsB << 2,
	    dvar_list_susp(Xs, Xs2, none, Goal, 2, Susp, [dom(Index),dom(Value)]),
	    fd_global(Goal, f(Index-IndexM,Value-ValueM,Xs2,Options,_Handle,0), Susp)
	).
...
element(Xs, Value) :-
	Goal = element(Xs, Value),
	must_be_dvar_list(Xs, Goal, 1),
	length(Xs, NIn),
	NIn > 0,
	(   ground(Xs) ->
	    list_to_fdset(Xs, Set),
	    Value in_set Set
	;   arg_attribute(Value, ValueM, Goal, 2),
	    dvars_are_bool([Value|Xs], IsB),
	    dvar_list_susp(Xs, Xs2, none, Goal, 2, Susp, [dom(Value)]),
	    fd_global(Goal, f(Value-ValueM,Xs2,IsB,NIn/*can overlap*/,_Handle,0), Susp)
	).
...
assignment(Xs, Ys) :-
        OptionsArgNo = 0,
	assignment(Xs, Ys, [], opt(global,dom,[],[],0,false,false), assignment(Xs,Ys,[]), OptionsArgNo).

assignment(Xs, Ys, Options) :-
        OptionsArgNo = 0,
	assignment(Xs, Ys, Options, opt(global,dom,[],[],0,false,false), assignment(Xs,Ys,Options), OptionsArgNo).
...
assignment(Xs, Ys, Options, Opt0, Goal, OptionsArgNo) :-
	...
	domain(Xs, Set),
	domain(Ys, Set),
	Opt = opt(Cons,On,Cost,_,_,Circuit,SubCircuit),
	...
	(   foreach(X,Xs),
	    foreach(Y,Ys),
	    foreach(none(X),SuspNoneX),
	    foreach(none(Y),SuspNoneY)
	do  true
	),
	(   foreach(X,Xs),
	...
	->  Flag = Flag0,
	    (   N<2 -> true
	    ;   fd_global_internal(subcircuit(Xs), state(XVec,0,_Handle2,0), SuspNoneX,
				   _, true, 4), % non-idempotent
		fd_global_internal(subcircuit(Ys), state(YVec,0,_Handle3,0), SuspNoneY,
				   _, true, 4) % non-idempotent
	    )
	;   Circuit = true
	->  Flag is Flag0+1,
	    ...
	    ;   not_self(Xs, 0),
		not_self(Ys, 0),
		fd_global_internal(circuit(Xs), state(XVec,0,_Handle2,0), SuspNoneX,
				   _, true, 4), % non-idempotent
		fd_global_internal(circuit(Ys), state(YVec,0,_Handle3,0), SuspNoneY,
				   _, true, 4) % non-idempotent
	    )
	;   Flag = Flag0
	),
	...
	->  fd_global_internal(assignment(_,_,_),
			       ...
			       Global, true, 0),
	    Global = global(StateM,_,_,_,_,_),
	    (   foreach(X2,Xs),
		count(I,0,_),
		param(C,StateM,Goal)
...
/****************************************************************/
/* symmetric_all_different/1, symmetric_all_distinct/1   	*/
/****************************************************************/

symmetric_all_different(Xs) :-
	assignment(Xs, Xs, [consistency(local)]).

symmetric_all_distinct(Xs) :-
	assignment(Xs, Xs, [consistency(global)]).

/****************************************************************/
/* circuit/[1,2], subcircuit/[1,2]               		*/
/****************************************************************/

circuit(Xs) :-
	all_different(Xs, [], opt(global,dom,[],[],0,true,false), circuit(Xs), 0).

circuit(Xs, Ys) :-
	assignment(Xs, Ys, [], opt(global,dom,[],[],0,true,false), circuit(Xs,Ys), 0).

subcircuit(Xs) :-
	all_different(Xs, [], opt(global,dom,[],[],0,false,true), subcircuit(Xs), 0).

subcircuit(Xs, Ys) :-
	assignment(Xs, Ys, [], opt(global,dom,[],[],0,false,true), subcircuit(Xs,Ys), 0).
...
/****************************************************************/
/* sum/3, scalar_product/[4,5], scalar_product_reif/[5,6]       */
/****************************************************************/
...
scalar_product(Cs, Xs, Rel, S) :-
	scalar_product_reif(Cs, Xs, Rel, S, 1, []).

scalar_product(Cs, Xs, Rel, S, Options) :-
	scalar_product_reif(Cs, Xs, Rel, S, 1, Options).

scalar_product_reif(Cs, Xs, Rel, S, Reif) :-
	scalar_product_reif(Cs, Xs, Rel, S, Reif, []).

scalar_product_reif(Cs, Xs, Rel, S, Reif, Options) :-
	Goal = scalar_product_reif(Cs,Xs,Rel,S,Reif,Options),
	Goal5 = scalar_product_reif(Cs,Xs,Rel,S,Reif),
	...
	(Reif==1 ; Cons = bound, Among = []), !,
	must_be_dvar_list(Xs, Goal, 2),
	must_be_dvar_list([S], Goal, 4),
	must_be_dvar_list([Reif], Goal, 5),
	arg_attribute(Reif, ReifA, Goal, 5),
	(Reif==1 -> true ; propagate_interval(Reif, 0, 1)),	
	(   foreach(X,Xs),
	    foreach(C,Cs),
	    foreach(X-C,L1)
	do  true
	...
	(   Reif==1, Rel = #=
	->  scalar_bound_sum(Cs1, Xs1, 0, 0, S) % try to bound S if unbounded
	;   Reif==0, Rel = #\=
	->  scalar_bound_sum(Cs1, Xs1, 0, 0, S) % try to bound S if unbounded
	;   true
	),
	scalar_state([-1|Cs1], [S|Xs1], Rel, 0, Vec, Sum, Susp, Goal),
	length(Vec, N),
	(   Reif==1,
	    \+'$fd_coref'(Susp),
	    sp_strength_reduce(N, Vec, Rel, Sum, [Kern], []) -> call(Kern)
	;   reif_sp_strength_reduce(N, Vec, Rel, Sum, Reif, [Kern], []) -> call(Kern)
	;   scalar_op(Rel, Op0),
	    scalar_op_fast(Vec, Sum, Op0, Op),
	    (   Options\==[] -> Pretty = Goal
	    ;   pretty_scalar_product(Goal5, Pretty)
	    ),
	    fd_global_internal(Goal5, state(Vec,Op,Sum,Reif-ReifA,0/*Nground/Trail*/,_,0), Susp,
			       _, clpfd:Pretty, 0)
	),
	% (   var(Op) -> true
...
scalar_product_reif(Cs, Xs, Rel, S, Reif, Options) :-
	propagate_interval(Reif, 0, 1),
	scalar_product([-1|Cs], [T|Xs], #=, 0, Options),
	sp_binary_reif(Rel, T, S, Reif).

sp_binary_reif(#=, T, S, Reif) :-
	'x=y'(T, S, Reif).
sp_binary_reif(#\=, T, S, Reif) :-
	'x\\=y'(T, S, Reif).
sp_binary_reif(#<, T, S, Reif) :-
...
pretty_scalar_product(scalar_product_reif(As,Xs,Rel,S0,Reif), Pretty) :-
...
:- sp_strength_reduce/6 is nondet.
sp_strength_reduce(N, V, #<, S) --> !,
	{S1 is S-1},
	sp_strength_reduce(N, V, #=<, S1).
sp_strength_reduce(N, V, #>, S) --> !,
	{S1 is S+1},
	sp_strength_reduce(N, V, #>=, S1).
sp_strength_reduce(0, [], Rel, S) --> !,
	sp_strength_reduce_0(Rel, S).
sp_strength_reduce(1, [f(C1,X1,_M1)], Rel, S) --> !,
	(   {Rel = #=<, C1 > 0} ->
	    {floordiv(S, C1, S1)},
	    ['t=<c'(X1, S1)]
	;   {Rel = #=<, C1 < 0} ->
...
sp_strength_reduce(_, Fs, #\=, S) --> % general disequation
	{'$fd_debug'(off, off, 1)},
	(   foreach(f(C,X,M),Fs),
	    foreach(C,Cs),
	    foreach(X,Xs),
...
sp_strength_reduce(2, [f(C1,X1,_M1),f(C2,X2,_M2)], Rel, S) -->
	(   {Rel = #=<} -> sp_strength_reduce_2_le(C1, X1, C2, X2, S)
	;   {Rel = #>=} -> sp_strength_reduce_2_ge(C1, X1, C2, X2, S)
	;   {Rel = #=} -> sp_strength_reduce_2_eq(C1, X1, C2, X2, S)
	;   {Rel = #\=} -> sp_strength_reduce_2_ne(C1, X1, C2, X2, S)
	).	
sp_strength_reduce(3, [f(C1,X1,_M1),f(C2,X2,_M2),f(C3,X3,_M3)], #=, S) -->
	sp_strength_reduce_3_eq(C1, X1, C2, X2, C3, X3, S).
% reduce to bool_or, bool_and
sp_strength_reduce(N, Fs, Rel, S) -->
	(   foreach(f(A,X,_),Fs),
	    foreach(PX,PXs),
	    foreach(NX,NXs),
	    fromto(0,NNeg0,NNeg1,NNeg)
	do  {fd_min_max(X, 0, 1)},
	    {   A = 1
	    ->  PX = X, NX = (#\ X), NNeg1 = NNeg0
	    ;   A = -1
	    ->  PX = (#\ X), NX = X, NNeg1 is NNeg0+1
	    }
	), !,
	(   {Rel = #>=, S+NNeg =:= 1  } -> [ bool_or(PXs,1)]
	;   {Rel = #=<, S+NNeg =:= N-1} -> [ bool_or(NXs,1)]
	;   {Rel = #>=, S+NNeg =:= N  } -> [bool_and(PXs,1)]
	;   {Rel = #=<, S+NNeg =:= 0  } -> [bool_and(NXs,1)]
	;   {Rel = #= , S+NNeg =:= N  } -> [bool_and(PXs,1)]
	;   {Rel = #= , S+NNeg =:= 0  } -> [bool_and(NXs,1)]
	).
...
reif_sp_strength_reduce(0, [], Rel, S, Reif) -->
	reif_sp_strength_reduce_0(Rel, S, Reif).
reif_sp_strength_reduce(1, [f(C1,X1,_M1)], Rel, S, Reif) -->
	reif_sp_strength_reduce_1(Rel, C1, X1, S, Reif).
reif_sp_strength_reduce(2, [f(C1,X1,_M1),f(C2,X2,_M2)], Rel, S, Reif) -->
	{C1 = 1, C2 = -1, S = 0},
	reif_sp_strength_reduce_2(Rel, X1, X2, Reif).
% reduce to bool_or, bool_and
reif_sp_strength_reduce(N, Fs, Rel, S, Reif) -->
	(   foreach(f(A,X,_),Fs),
	    foreach(PX,PXs),
	    foreach(NX,NXs),
	    fromto(0,NNeg0,NNeg1,NNeg)
	...
	(   {Rel = #>=, S+NNeg =:= 1  } -> [ bool_or(PXs,Reif)]
	;   {Rel = #=<, S+NNeg =:= N-1} -> [ bool_or(NXs,Reif)]
	;   {Rel = #>=, S+NNeg =:= N  } -> [bool_and(PXs,Reif)]
	;   {Rel = #=<, S+NNeg =:= 0  } -> [bool_and(NXs,Reif)]
	;   {Rel = #= , S+NNeg =:= N  } -> [bool_and(PXs,Reif)]
	;   {Rel = #= , S+NNeg =:= 0  } -> [bool_and(NXs,Reif)]
	).

reif_sp_strength_reduce_0(#<, S, Reif) --> ({S > 0} -> [Reif=1] ; [Reif=0]).
reif_sp_strength_reduce_0(#=<, S, Reif) --> ({S >= 0} -> [Reif=1] ; [Reif=0]).
reif_sp_strength_reduce_0(#>, S, Reif) --> ({S < 0} -> [Reif=1] ; [Reif=0]).
reif_sp_strength_reduce_0(#>=, S, Reif) --> ({S =< 0} -> [Reif=1] ; [Reif=0]).
reif_sp_strength_reduce_0(#=, S, Reif) --> ({S =:= 0} -> [Reif=1] ; [Reif=0]).
...
reif_sp_strength_reduce_1(#=<, C1, X1, S, Reif) -->
	{C1 > 0}, !,
	{floordiv(S, C1, S1)},
	['x=<y'(X1, S1, Reif)].
reif_sp_strength_reduce_1(#=<, C1, X1, S, Reif) -->
...
reif_sp_strength_reduce_2(#=, T, S, Reif) -->
	['x=y'(T, S, Reif)].
reif_sp_strength_reduce_2(#\=, T, S, Reif) -->
	['x\\=y'(T, S, Reif)].
reif_sp_strength_reduce_2(#<, T, S, Reif) -->
...
scalar_bound_sum([C|Cs], [X|Xs], Min0, Max0, S) :-
	...
	scalar_bound_sum(Cs, Xs, Min2, Max2, S).
scalar_bound_sum(_, _, _, _, _).
...
scalar_state(Cs, Xs, Rel, S, Vec, Sum, Susp, Goal) :-
	...
	;   Vec0=[f(GCD0,_,_)|_],
	    ...
	    scalar_state_adjust(Rel, Sum0, GCD, Sum, Vec1, Vec)
	).

scalar_state_adjust(#<, Sum0, GCD, Sum, Vec, Vec) :-
...
scalar_state_adjust(#>=, Sum0, GCD, Sum, Vec, Vec) :-
	ceildiv(Sum0, GCD, Sum).
scalar_state_adjust(#=, Sum0, GCD, Sum, Vec, Vec) :- % #=, disentailed if =\= (modulo GCD)
	Sum0 mod GCD =:= 0, !,
	Sum is Sum0//GCD.
scalar_state_adjust(#=, _, _, 1, _, []). % #=, disentailed if =\= (modulo GCD)
scalar_state_adjust(#\=, Sum0, GCD, Sum, Vec, Vec) :- % #\=, entailed if =\= (modulo GCD)
	Sum0 mod GCD =:= 0, !,
	Sum is Sum0//GCD.
scalar_state_adjust(#\=, _, _, 1, _, []). % #\=, entailed if =\= (modulo GCD)
...
equate_and_propagate(Constraint, Attv) :-
	'$fd_post_equality'(Constraint, Attv, RC),
	evaluate(RC, _).
...
'x\\=y'(T, U) :-
	'$fd_debug'(off, off, 1),
	get_atts(T, fd_attribute(_,TDomM,_,_,_)),
	get_mutable(TDom, TDomM),
	get_atts(U, fd_attribute(_,UDomM,_,_,_)),
...
in_set_iff_rt(X, Set, B) :-
	...
	Attv =  in_set_ix(_,_),
	arg(1, Attv, A),	% ensure arg 1 deref
	arg(2, Attv, Copy),	% ensure arg 2 deref
...
disjoint1(Items0, Options) :-
	Goal = disjoint1(Items0,Options),
	must_be(Options, proper_list, Goal, 2),
	disjoint1_options(Options, opt(0,inf,sup,[]), Opt, Goal, 2),
	disjoint1_normal_items(Items0, Items1),
	Opt = opt(Flags,Min,Max,Margins),
	(   Flags /\ 2 =:= 2
	->  aux1_rectangles(Items1, wrap(Min,Max), Items2, [])
	;   Items1 = Items2
	),
	(   foreach(M,Margins),
	    param(Items2)
	do  margin1_rectangles(M, Items2)
	),
	mkitems1_as_diffn(Items2, Objects1, Objects2, Goal, Susp, []),
	length(Objects2, NT),
	fd_global(diffn(Objects1,[]), f(1,Objects2,0,NT,_Handle,0), Susp).

disjoint1_normal_items(Items0, Items1) :-
	(   foreach(X,Items0),
	    foreach(r(O,D,T),Items1)
	do  arg(1,X,O),
	    arg(2,X,D),
	    (arg(3,X,T) -> true; T=0)
	).
...
aux1_rectangles([], _) --> [].
aux1_rectangles([r(X,W,T)|Rs], wrap(Xmin,Xmax)) -->
	{Xub is Xmax-1},
	{X in Xmin..Xub},
	{Dx #= Xmax-Xmin},
	{X1 #= X-Dx},
	...
	[r(X1,W,T),
	 r(X ,W,T),
	 r(X2,W,T)],
	aux1_rectangles(Rs, wrap(Xmin,Xmax)).

margin1_rectangles(Margin, Items) :-
	(   foreach(Item1,Items),
	    count(I,1,_),
	    param(Margin,Items)
	do  (   foreach(Item2,Items),
		count(J,1,_),
		param(Margin,Item1,I)
	    do  (   I \== J,
		    Item1 = r(O1,D1,T1),
		    Item2 = r(O2,D2,T2),
		    Margin = margin(T1,T2,Diff)
		->  (   O1+D1+Diff #=< O2 #\/
			O2+D2+Diff #=< O1
		    )
		;   true
...
mkitems1_as_diffn([], [], [], _) --> [].
mkitems1_as_diffn([r(O,D,_)|Xs], [[O-D]|Objects1], [[[(O-OM),(D-DM)]]|Objects2], Goal) -->
	[none(O),none(D)],
	{finite_arg_attribute(O, OM, Goal, 1)},
	{finite_arg_attribute(D, DM, Goal, 1)},
	mkitems1_as_diffn(Xs, Objects1, Objects2, Goal).
...
disjoint2(Items0, Options) :-
	Goal = disjoint2(Items0,Options),
	must_be(Options, proper_list, Goal, 2),
	disjoint2_options(Options, opt(0,inf,sup,inf,sup,[]), Opt, Goal, 2),
	disjoint2_normal_items(Items0, Items1),
	Opt = opt(Flags,Min1,Max1,Min2,Max2,Margins),
	(   Flags /\ 6 =\= 0
	->  aux2_rectangles(Items1, wrap(Min1,Max1,Min2,Max2), Items2, [])
	;   Items1 = Items2
	),
	(   foreach(M,Margins),
	    param(Items2)
	do  margin2_rectangles(M, Items2)
	),
	mkitems2_as_diffn(Items2, Objects1, Objects2, Goal, Susp, []),
	length(Objects2, NT),
	fd_global(diffn(Objects1,[]), f(2,Objects2,0,NT,_Handle,0), Susp).

disjoint2_normal_items(Items0, Items1) :-
	(   foreach(X,Items0),
	    foreach(r(O1,D1,O2,D2,T),Items1)
	do  arg(1,X,O1),
	    arg(2,X,D1),
	    arg(3,X,O2),
	    arg(4,X,D2),
...
aux2_rectangles([], _) --> [].
aux2_rectangles([r(X,W,Y,H,T)|Rs], wrap(Xmin,Xmax,Ymin,Ymax)) -->
	{Xub is Xmax-1},
	{X in Xmin..Xub},
	{Yub is Ymax-1},
	{Y in Ymin..Yub},
	...
	[r(X1,W,Y1,H,T),
	 ...
	 r(X ,W,Y2,H,T),
	 r(X2,W,Y2,H,T)],
	aux2_rectangles(Rs, wrap(Xmin,Xmax,Ymin,Ymax)).

margin2_rectangles(Margin, Items) :-
	(   foreach(Item1,Items),
	    count(I,1,_),
	    param(Margin,Items)
	do  (   foreach(Item2,Items),
		...
		param(Margin,Item1,I)
	    do  (   I \== J,
		    Item1 = r(X1,W1,Y1,H1,T1),
		    Item2 = r(X2,W2,Y2,H2,T2),
		    Margin = margin(T1,T2,XD,YD)
		->  (   X1+W1+XD #=< X2 #\/
		        Y1+H1+YD #=< Y2 #\/
		        X2+W2+XD #=< X1 #\/
		        Y2+H2+YD #=< Y1
...
mkitems2_as_diffn([], [], [], _) --> [].
mkitems2_as_diffn([r(X,W,Y,H,_)|Rs], [[X-W,Y-H]|Objects1], [[[(X-XM),(W-WM)],[(Y-YM),(H-HM)]]|Objects2], Goal) -->
	[none(X),none(W),none(Y),none(H)],
	{finite_arg_attribute(X, XM, Goal, 1)},
	{finite_arg_attribute(W, WM, Goal, 1)},
	{finite_arg_attribute(Y, YM, Goal, 1)},
	{finite_arg_attribute(H, HM, Goal, 1)},
	mkitems2_as_diffn(Rs, Objects1, Objects2, Goal).
...
disjoint2_option(global(B), _,	% obsolescent since SP 4.6
...
disjoint2_option(margin(T1,T2,D1,D2), _,
	...
	Flags is (Flags0 /\ -9) \/ 8.
disjoint2_option(synchronization(B), _, % obsolescent since ever
		 opt(Flags0,Min1,Max1,Min2,Max2,Ms),
		 opt(Flags,Min1,Max1,Min2,Max2,Ms)) :-
	bool_option(B, Value),
...
diffn(Objects, Options) :-
	Goal = diffn(Objects,Options),
	must_be(Objects, proper_list(proper_list), Goal, 1),
	diffn_options(Options, opt(0), opt(Strict), Goal, 2),
	(   foreach(Object,Objects),
	    foreach(Object1,Objects1),
	    fromto(Susp1,Susp2,Susp5,[]),
	    param(Goal,KDims)
	do  must_be(Object, proper_list(pair), Goal, 1),
	    length(Object, KDims),
	    (   foreach(Orig-Size,Object),
		foreach(Facet,Object1),
		fromto(Susp2,Susp3,Susp4,Susp5),
		param(Goal)
	    do  must_be_list_of_finite_dvar([Orig,Size], Goal, 1),
		propagate_interval(Size, 0, sup),
		dvar_list_susp([Orig,Size], Facet, none, Goal, 1, Susp3, Susp4)
	    )
	),
	length(Objects, NT),
	fd_global(Goal, f(KDims,Objects1,Strict,NT,_Handle,0), Susp1).

diffn_options([], Opt, Opt, _, _).
diffn_options([X|L], Opt0, Opt, Goal, ArgNo) :-
	(   nonvar(X),
	    diffn_option(X, Opt0, Opt1) -> true
        ;   illarg(domain(term,diffn_option), Goal, ArgNo, X)
        ),
	diffn_options(L, Opt1, Opt, Goal, ArgNo).

diffn_option(strict(B), opt(_), opt(F)) :-
	bool_option(B, F).
...
global_cardinality(Vars, Vals1, Options, Opt0, Goal) :-
	...
	dvar_list_susp(Vars, XVec, On, Goal, 1, Susp, Susp3),
	(   foreach(C1,Counts),
	...
	(   Flag=\=0 ->
	    ...
	    fd_global_internal(Goal,
			       f(N,M,XVec,SVals,Flag,Cost,0/*cost so far*/,_Handle,0),
	...
	;   Cons==local ->
	    ...
	    fd_global_internal(local_cardinality,
			       f(N,M,XVec,SVals,_Handle,0),
			       Susp, _Global, clpfd:Goal, 0)
	;   Susp4=[],
	    fd_global(Goal, f(N,M,XVec,SVals,Flag,Cost,0/*cost so far*/,_Handle,0), Susp)
	).
...
/****************************************************************/
/* minimum_arg/2, maximum_arg/2					*/
/****************************************************************/

minimum_arg(Xs, Index) :-
	Goal = minimum_arg(Xs, Index),
	must_be_dvar_list(Xs, Goal, 1),
	length(Xs, N),
	propagate_interval(Index, 1, N),
	dvars_are_bool(Xs, B),
	arg_attribute(Index, IndexM, Goal, 2),
	dvar_list_susp(Xs, Xs2, minmax, Goal, 1, Susp, [dom(Index)]),
	fd_global(Goal, f(Xs2,Index-IndexM,B,_Handle,0), Susp).

maximum_arg(Xs, Index) :-
	Goal = maximum_arg(Xs, Index),
	must_be_dvar_list(Xs, Goal, 1),
	length(Xs, N),
	propagate_interval(Index, 1, N),
	dvars_are_bool(Xs, B),
...
dvars_are_bool(Xs, B) :-
	(   foreach(X,Xs)
	do  fd_min_max(X, Min, Max),
	    zero_one_min_max(Min, Max)
	), !,
	B = 1.
dvars_are_bool(_, 0).

zero_one_min_max(0, 0).
zero_one_min_max(0, 1).
zero_one_min_max(1, 1).
...
lex_chain(Tuples, Options) :-
	...
	;   (   fromto(Tuples,[Tuple1,Tuple2|Ts],[Tuple2|Ts],[_]),
		...
		param(Options,NV,Flag,Among,FdGlobalOpt)
...
lex_chain_option(global(B), opt(F0,A), opt(F,A)) :- !,
	bool_option(B, Value),
	F is (F0/\0x17)\/(Value<<3).
lex_chain_option(hidden(B), opt(F0,A), opt(F,A)) :- !,
	bool_option(B, Value),
	F is (F0/\0x07)\/(Value<<4).
...
seq_precede_chain(Xs, Options) :-
	Goal = seq_precede_chain(Xs, Options),
	must_be(Xs, proper_list, Goal, 1),
	length(Xs, N),
	(   for(I,1,N),
	    foreach(I,Vs)
	...
	value_precede_chain(Vs, Xs, Options).
	
/****************************************************************/
/* value_precede_chain/3					*/
/****************************************************************/

value_precede_chain(Vs, Xs) :-
	value_precede_chain(Vs, Xs, []).

value_precede_chain(Vs, Xs, Options) :-
	Goal = value_precede_chain(Vs, Xs, Options),
	must_be(Vs, proper_list(integer), Goal, 1),
	must_be_dvar_list(Xs, Goal, 2),
	must_be(Options, proper_list, Goal, 3),
	value_precede_chain_options(Options, opt(false), opt(Global), Goal, 3),
	(   Vs = [] -> true
	;   Vs = [_] -> true
	;   Global = false ->
	    value_precede_chain_decomposed(Vs, Xs)
	;   finite_dvar_list_susp(Xs, Xs2, none, Goal, 2, Susp, []),
	    fd_global(Goal, f(Vs,Xs2,0,0,_Handle,0), Susp)
	).
...
value_precede_chain_decomposed(Vs, Xs) :-
	...
	automaton(Xs, [source(1)|Sinks], Arcs).

value_precede_chain_options([], Opt, Opt, _, _).
value_precede_chain_options([X|L], Opt0, Opt, Goal, ArgNo) :-
	(   nonvar(X),
	    value_precede_chain_option(X, Opt0, Opt1) -> true
        ;   illarg(domain(term,value_precede_chain_option), Goal, ArgNo, X)
        ),
	value_precede_chain_options(L, Opt1, Opt, Goal, ArgNo).

value_precede_chain_option(global(B), opt(_), opt(B)) :-
	bool_option(B, _).
...
all_equal_reif(Xs, Reif) :-
	Reif==1, !,
	all_equal(Xs).
all_equal_reif(Xs, Reif) :-
	Goal = all_equal_reif(Xs, Reif),
	must_be_dvar_list(Xs, Goal, 1),
	must_be_dvar_list([Reif], Goal, 2),
	propagate_interval(Reif, 0, 1),
	arg_attribute(Reif, ReifM, Goal, 2),
	dvar_list_susp(Xs, Xs2, dom, Goal, 1, Susp, [val(Reif)]),
	fd_global(Goal, f(Xs2,Reif-ReifM,0,_Handle,0), Susp).

all_equal(Xs) :-
	Goal = all_equal_reif(Xs),
	must_be_dvar_list(Xs, Goal, 1),
	all_equal_checked(Xs).

all_equal_checked([]) :- !.
all_equal_checked([_]) :- !.
all_equal_checked([X,Y|Ys]) :-
	'x=y'(X, Y),
	all_equal_checked([Y|Ys]).
