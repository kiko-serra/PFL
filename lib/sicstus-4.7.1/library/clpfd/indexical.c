...
void fd_tell_value(Wam wam, TAGGED value)
{
  ...
  if (TagToSTR(domain) >= w->global_uncond && TagToLST(DomainSet(domain)) >= w->global_uncond) { /* can safely smash domain _and set_ */
    ...
    fd_update_mutable(wam, MakeStructure(h+2), mutable);
  }
  fd_told(wam, why);
}
...
void fd_tell(Wam wam, TAGGED new)
{
  ...
  if (CTagToCdr(new)==EmptySet) { /* new domain is an interval */
	...
	fd_update_mutable(wam, MakeStructure(h), mutable);
      }
    }
  }
  SP_ASSERT(card!=TaggedOne);
				...
				SP_term_ref GlobalR)
{
  ...
  while (TagIsLST(X(0))) {
	...
	if (IsVar(ent)) {	/* SPRM 12958 */
	  ...
	  fd_update_mutable(wam,status,statmut);
	}
      }
    } else {
...
static void
sync_status(Wam wam, SP_BOOL entailed, TAGGED truth_value, struct indexical_info *current) {
  if (entailed) {
    TAGGED entvar;
    ...
    fd_update_mutable(wam,gtemp,X(1));
  }
}
...
eval_indexical(Wam wam, SP_term_ref Global)
{
 ...
 abort:
  sync_status(wam, ground, truth_value, current);
...
prolog_fd_evaluate_indexical(Wam wam, SP_term_ref Global)
{
 ...
 restart:
  ...
  case 2:
    ...
    fd_update_mutable(wam,gtemp|IStep(8), statmut); /* STATUS: current */
    fd.resumptions++;
    ...
    fd_update_mutable(wam,RefTerm(State),statem);
