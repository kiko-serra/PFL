		  ...
		  TAGGED trigger)
{
  ...
  if (TagToSTR(lists) < w->global_uncond)/* can't smash */ {
    ...
    for (s = TagToSTR(lists), i=0; i<FD_LISTS_SIZE; i++)
      *h++ = *s++;
    fd_update_mutable(wam,new_lists,mutable);
    lists = new_lists;
  }
  if (key==functor_dom1) {
    mask = IStep(MASK_DOM);
...
prolog_fd_begin(Wam wam)
{
  ...
  if (!fd.batching) {
    ...
    cur->next = fd.current_propagator;
    fd.current_propagator = cur;
    fd_update_mutable(wam,PointerToTerm(cur),CLPFD_MUTABLE);
  }
}
...
void fd_end(Wam wam)
{
  ...
  if (!fd.batching) {
    ...
    cur->next = fd.free_propagators;
    fd.free_propagators = cur;
    fd_update_mutable(wam,PointerToTerm(fd.current_propagator),CLPFD_MUTABLE);
  }
}
...
static void fd_enqueue_list_gc_global_or_ix(Wam wam, struct propagator *cur, int index, int queue)
{
  TAGGED list_term = RefMutable(PROTECTED_LISTS_MUTABLE);
  TAGGED list = CTagToArg(list_term,index);
  ...
  while (list!=EmptySet) {
	...
	if ((status & IStep(0x60)) == IStep(0x60)) {
	  ...
	  if (DomainMin(reif_dom) != TaggedOne)
	    ...
	    fd_update_mutable(wam,status & ~IStep(0x60),status_mut); /* STATUS: ~guarded indexical */
	} else {
...
/* ASSERT: no GC in scope of this */
static void fd_enqueue_list_global_or_ix(Wam wam, struct propagator *cur, int index, int queue)
{
  TAGGED list = CTagToArg(RefMutable(PROTECTED_LISTS_MUTABLE),index);
...
void fd_enqueue_all(Wam wam, int bits, TAGGED lists_loc, TAGGED value)
{
  ...
  if (bits & MASK_SINGLETON) {
    ...
    if (bits & MASK_DOM) {
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_DOM_IX, FD_QUEUE_IX);
      fd_enqueue_list_daemon(wam, cur, FD_LIST_DOM_DAEMON);
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_DOM_GLOBAL, FD_QUEUE_DOM);
    }
    if (bits & MASK_MIN) {
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_MIN_IX, FD_QUEUE_IX);
      fd_enqueue_list_daemon(wam, cur, FD_LIST_MIN_DAEMON);
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_MIN_GLOBAL, FD_QUEUE_MINMAX);
    }
    if (bits & MASK_MAX) {
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_MAX_IX, FD_QUEUE_IX);
      fd_enqueue_list_daemon(wam, cur, FD_LIST_MAX_DAEMON);
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_MAX_GLOBAL, FD_QUEUE_MINMAX);
    }
    if (bits & MASK_MINMAX) {
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_MINMAX_IX, FD_QUEUE_IX);
      fd_enqueue_list_daemon(wam, cur, FD_LIST_MINMAX_DAEMON);
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_MINMAX_GLOBAL, FD_QUEUE_MINMAX);
    }
    if (bits & MASK_VAL) {
      TAGGED list;
      if (value == TaggedZero) {
	...
	if (list!=EmptySet) {
      ...
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_VAL_IX, FD_QUEUE_IX);
      fd_enqueue_list_daemon(wam, cur, FD_LIST_VAL_DAEMON);
      fd_enqueue_list_global_or_ix(wam, cur, FD_LIST_VAL_GLOBAL, FD_QUEUE_VAL);
    }
  } else {
    /* append slowest last! */
    if (bits & MASK_MINMAX) {
      fd_enqueue_list_gc_global_or_ix(wam, cur, FD_LIST_MINMAX_IX, FD_QUEUE_IX);
      fd_enqueue_list_gc_daemon(wam, cur, FD_LIST_MINMAX_DAEMON);
      fd_enqueue_list_gc_global_or_ix(wam, cur, FD_LIST_MINMAX_GLOBAL, FD_QUEUE_MINMAX);
    }
    if (bits & MASK_MAX) {
      fd_enqueue_list_gc_global_or_ix(wam, cur, FD_LIST_MAX_IX, FD_QUEUE_IX);
      fd_enqueue_list_gc_daemon(wam, cur, FD_LIST_MAX_DAEMON);
      fd_enqueue_list_gc_global_or_ix(wam, cur, FD_LIST_MAX_GLOBAL, FD_QUEUE_MINMAX);
    }
    if (bits & MASK_MIN) {
      fd_enqueue_list_gc_global_or_ix(wam, cur, FD_LIST_MIN_IX, FD_QUEUE_IX);
      fd_enqueue_list_gc_daemon(wam, cur, FD_LIST_MIN_DAEMON);
      fd_enqueue_list_gc_global_or_ix(wam, cur, FD_LIST_MIN_GLOBAL, FD_QUEUE_MINMAX);
    }
    if (bits & MASK_DOM) {
      fd_enqueue_list_gc_global_or_ix(wam, cur, FD_LIST_DOM_IX, FD_QUEUE_IX);
      fd_enqueue_list_gc_daemon(wam, cur, FD_LIST_DOM_DAEMON);
      fd_enqueue_list_gc_global_or_ix(wam, cur, FD_LIST_DOM_GLOBAL, FD_QUEUE_DOM);
    }
  }
}
...
prolog_fd_global_enqueue(Wam wam, SP_term_ref TermR)
{
  ...
  fd_update_mutable(wam,RefMutable(mutable)|IStep(9),mutable); /* STATUS: enqueued, current */
  /*** done in Prolog
  fd_sync(wam);
...
struct instance *get_dbref(Wam wam, TAGGED ref)
{
  DerefNonvar(ref);
...
#else
  {
    struct instance * ins = sp_decode_dbref(ref); /* 4.7.0+ New API. */
    SP_ASSERT(ins != NULL); /* FIXME: How handle? */
    return ins;
  }
#endif
}
...
fd_daemon_copy_state(Wam wam, TAGGED *global, SP_BOOL *buried)
{
  ...
  if (s >= w->global_uncond) {
    ...
    *buried = TRUE;
    ...
    fd_update_mutable(wam,tstate,CTagToArg(*global,1));
  }
  return tstate;
}
